<?php
	...
	{
		$msg="Receipt Entry Saved Successfully!!!";
	}
	if(isset($_REQUEST['s1']))
	{
		...
		$rid="RC".$id;
		mysqli_query($con,"insert into receipt set rec_id=$id, rdate='$sdate', party='$str[0]', remark='$_REQUEST[remark]', paidby='$paidby', amount='$amt', cheque='$_REQUEST[cheque]', relatedwith='$rid', recno='$_REQUEST[recno]'");
		
		$cmp1=mysqli_query($con,"select max(trans_id) from transaction");
		$cmp=mysqli_fetch_row($cmp1);
		$tid=$cmp[0]+1;
		
		mysqli_query($con,"insert into transaction set trans_id='".$tid."', tdate='".$sdate."', ledger_id='".$paidby."', amount='".$amt."', particulars='Dues Received From - $a[0]', type='Dr.', relatedto='$rid'");		
		$tid++;
		
		mysqli_query($con,"insert into transaction set trans_id='".$tid."', tdate='".$sdate."', ledger_id='".$str[0]."', amount='".($amt)."', particulars='Dues Paid', type='Cr.', relatedto='$rid'");
		$tid++;
		echo "<script language=\"javascript\">window.open(\"printreceipt.php?rec_id=$id\",\"_blank\");</script>";
		echo "<script language=\"javascript\">window.open(\"addreceipt.php?msg=set\",\"_self\");</script>";
	}	
?>
<!DOCTYPE html>
<html lang="en">
...
function getbal(val)
{
    ...
    <body onload="startc();">
        ...
        <div class="page-container">
            ...
            <!-- START PAGE SIDEBAR -->
             <?php $menu12=true; $smenu12="1";  include_once("sidebar.php"); ?>
            <!-- END PAGE SIDEBAR -->
                ...
                <div class="page-content-wrap">
                    ...
                    <div class="row">
                        <div class="col-md-12">
							...
							?>
                            <form class="form-horizontal" method="post" action="addreceipt.php" enctype="multipart/form-data">
                            <div class="panel panel-default">
                                <div class="panel-heading">
									
									 <h3 class="panel-title"><strong>Add </strong> Receipt</h3>
                                </div>
								<?php
									$date=date("Y-m-d");
								?>
                                <div class="panel-body">   
									<?php
										$d=mysqli_fetch_row(mysqli_query($con,"select max(recno) from receipt"));
										$d[0]++;
									?>
									<div class="form-group">
                                        <label class="col-md-3 col-xs-12 control-label">Receipt No.</label>
                                        <div class="col-md-6 col-xs-12">                                            
											<?php
												$invno=mysqli_query($con,"select recno,rec_id from receipt order by rec_id desc limit 1");
												
												if($i=mysqli_fetch_row($invno))
													$i[0]=explode("/",$i[0])[3];
												else $i[0]=$i[1]=0;
												$i[0]++;
												if($i[0]<10) $no="EDS/R/".date("m-y")."/00".$i[0];
												else if($i[0]<100) $no="EDS/R/".date("m-y")."/0".$i[0];
												else $no="EDS/R/".date("m-y")."/".$i[0];
											?>
											<input type="hidden" class="form-control" name="rec_id" value="<?php echo ++$i[1];?>"/>
											<input type="hidden" class="form-control" name="recno" value="<?php echo $no; ?>"/>
											<b><?php echo $no; ?></b>
                                        </div>
                                    </div> 
									<div class="form-group">
