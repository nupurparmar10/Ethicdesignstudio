<?php
	...
	{
		$msg="Sales Bill Modified Successfully!!!";
	}
	if(isset($_REQUEST['sale_id']))
	{
		...
		mysqli_query($con,"delete from billbook where sale_id='$_REQUEST[sale_id]'");
		
		$msg1="Sales Bill Deleted Successfully!!!";
	}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>        
	...
	{
		var path="viewsales.php?sale_id="+row;
    ...
    <body>
        ...
        <div class="page-container">
            ...
            <!-- START PAGE SIDEBAR -->
             <?php  $menu10=true; $smenu10="5"; include_once("sidebar.php"); ?>
            <!-- END PAGE SIDEBAR -->
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body">
                                   <form class="form-horizontal" method="post" action="viewsales.php" name='frm2' enctype="multipart/form-data">
  ...
  if(isset($_REQUEST['open']))
  {
	...
	$sql="select * from billbook where invno like '%$_REQUEST[invno]%' ".$paidby." ".$dfrom." ".$dto." ".$party." ".$taxtype." ".$emp_id." order by invdate desc,sale_id desc";
			...
			?>
				 <tr id="<?php echo $d[0]; ?>">
					...
					<?php
						...
						if($d[7]!="Credit")
						{
							$str=mysqli_fetch_row(mysqli_query($con,"select name from ledger_accounts where ledger_id='$d[7]'"));
							$d[7]=$str[0];
						}
						$emp1=mysqli_query($con,"select empname from empdet where ledger_id='$d[17]'");
						if($emp=mysqli_fetch_row($emp1)) {} else $emp[0]="";
						$comm=$d[4]*($d[14]+$d[9]-$d[10]-$d[11]-$d[12]+$d[13])/100;
					?>
					<td><?php echo $p[0]; ?></td>
					<td><?php echo $d[3]; ?></td>
					<td><?php echo $d[7]; ?></td>
					<td><?php echo $d[5]; ?></td>
					<td align='right'><?php echo number_format($d[14],2); ?></td>
					<td><?php echo $d[16]; ?></td>
					...
					<?php
						$table .= "<td>$p[0]</td>
								...
								<td>$d[7]</td>
								<td>$d[5]</td>
								<td align='right'>".number_format($d[14],2)."</td>
								<td>$d[16]</td>
								<td>$emp[0]</td>
					...
					?>
					<td>
						<button class="btn btn-info btn-rounded btn-condensed btn-sm" onClick="window.open('addsales.php?sale_id=<?php echo $d[0]; ?>','_self');"><span class="fa fa-pencil"></span></button>
