...
if (isset($_REQUEST['status_id']) && isset($_REQUEST['status'])) {
	$r_id = intval($_REQUEST['status_id']);
	$status = ($_REQUEST['status'] == '1') ? 1 : 0;

	mysqli_query($con, "UPDATE reviews SET status = $status WHERE r_id = $r_id");

	$msg = "Review status updated to " . ($status ? "Show" : "Hide");
}
if (isset($_REQUEST['r_id'])) {
	mysqli_query($con, "delete from reviews where r_id='$_REQUEST[r_id]'");
	$msg1 = "Review Deleted successfully!!!";
}
...
<head>
	...
	<script type="text/javascript" language="javascript">
		...
		function delete_row1(row) {
			var path = "upreview.php?r_id=" + row;
			window.open(path, "_self");
		}

		function changeReviewStatus(id, status) {
			if (confirm("Are you sure you want to update the status?")) {
				const url = `upreview.php?status_id=${id}&status=${status}`;
						...
						<div class="panel panel-default">
							...
							<div class="panel-body">
								...
								<div class="table-responsive" id="display">
									<?php
									$sql = "SELECT * FROM reviews order by r_id";

									$result = mysqli_query($con, $sql);


									if (mysqli_num_rows($result) == 0) {
										echo "There is no review Available!!!";
									} else {
										$table = "";
										$table = "<table width='100%' border='1' align='center' style='border-collapse:collapse;'>
													<caption><h1>Review Details</h1></caption>
													<tr>
														...
														<th width='150'><span>Message</span></th>	
														<th width='150'><span>Product</span></th>	
													</tr>";
									?>
										<table class="table datatable table-bordered table-striped table-actions">
											<thead>
												<tr>
													<th width="58"><span>S. No.</span></th>
													<th width="189"><span>Name</span></th>
													<th width="141"><span>Email</span></th>
													<th width="155"><span>Title</span></th>
													<th width='150'><span>Message</span></th>
													<th width='150'><span>Product</span></th>	
											...
											<tbody>
												...
												if ($row = mysqli_fetch_row($result)) {
													$j = 1;
													do {
														$i=mysqli_fetch_row(mysqli_query($con,"Select * from item_details where item_id='$row[8]'"));
												?>
														<tr id="<?php echo $row[0]; ?>">
															...
															$table .=  "<tr>
																						...
																						<td style='padding-left:10px;'>" . htmlspecialchars($i[1]) . "-". htmlspecialchars($i[5]) . "</td>
																						</tr>";
															?>
															<td class="d-flex gap-5">
																...
																<button type="button" class="btn <?php echo $row[6] == 1 ? 'btn-success' : 'btn-warning'; ?> btn-sm" onclick="changeReviewStatus('<?php echo $row[0]; ?>', '<?php echo $row[6] == 1 ? 0 : 1; ?>')">
																	<span class="fa <?php echo $row[6] == 1 ? 'fa-toggle-on' : 'fa-toggle-off'; ?>"></span>
																</button>
															</td>
										...
										<div class="col-md-3 col-xs-3">
											<form action="excel.php" method="post">
												...
												<input type="hidden" name="fn" value="Review Details" />
