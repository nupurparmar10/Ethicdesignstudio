<?php
	...
	if(isset($_REQUEST['s1']))
	{
			...
			$cmp1=mysqli_query($con,"select max(exp_id) from expense_detail");
			$cmp=mysqli_fetch_row($cmp1);
			$id=$cmp[0]+1;
			
			$rid="E".$id;
			mysqli_query($con,"insert into expense_detail set exp_id=$id, expdate='$expdate', ledger_id='$ledger_id', paidby='$paidby', amount='$amt', remark='$_REQUEST[remark]', relatedwith='$rid', party='$_REQUEST[party]'");
			...
			{
				mysqli_query($con,"insert into transaction set trans_id='".$tid."', tdate='".$expdate."', ledger_id='".$_REQUEST['party']."', amount='".$amt."', particulars='Expense for $b[0] $_REQUEST[remark]', type='Cr.', relatedto='$rid'");		
				$tid++;
				
				mysqli_query($con,"insert into transaction set trans_id='".$tid."', tdate='".$expdate."', ledger_id='".$ledger_id."', amount='".$amt."', particulars='Expense paid $_REQUEST[remark]', type='Dr.', relatedto='$rid'");
				$tid++;
			}
			else
			{
				mysqli_query($con,"insert into transaction set trans_id='".$tid."', tdate='".$expdate."', ledger_id='".$paidby."', amount='".$amt."', particulars='Expense Paid to $b[0] $_REQUEST[remark]', type='Cr.', relatedto='$rid'");		
				$tid++;
				
				mysqli_query($con,"insert into transaction set trans_id='".$tid."', tdate='".$expdate."', ledger_id='".$ledger_id."', amount='".$amt."', particulars='Expense paid $_REQUEST[remark]', type='Dr.', relatedto='$rid'");
				$tid++;
			}
			$msg="Expense Entry Saved Successfully!!!";
	}
	if(isset($_REQUEST['s3']))
	{
			$pid1=mysqli_query($con,"select relatedwith from expense_detail where exp_id=$_REQUEST[exp_id]");
			...
			mysqli_query($con,"update expense_detail set expdate='$expdate', ledger_id='$ledger_id', paidby='$paidby', amount='$amt', remark='$_REQUEST[remark]', party='$_REQUEST[party]' where exp_id='$_REQUEST[exp_id]'");
			...
			if($paidby=="Credit")
			{
				mysqli_query($con,"insert into transaction set trans_id='".$tid."', tdate='".$expdate."', ledger_id='".$party."', amount='".$amt."', particulars='Expense for $b[0] $_REQUEST[remark]', type='Cr.', relatedto='$rid'");		
				$tid++;
			...
			header("Location: viewexpentry.php?msg=set"); die;
	}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>        
    ...
    <body>
        ...
        <div class="page-container">
            ...
            <!-- START PAGE SIDEBAR -->
             <?php $menu5=true; $smenu5="2";  $ssmenu5="21"; include_once("sidebar.php"); ?>
            <!-- END PAGE SIDEBAR -->
            ...
            <div class="page-content">
                ...
                <ul class="breadcrumb">
                   <li><a href="#" onclick="window.location=document.referrer;">Back</a></li>
                    ...
                    <li><a href="#">Expense Master</a></li>
					<li><a href="#">Expense Entry</a></li>
                    <li class="active"><?php if(isset($_REQUEST['exp_id'])) {?>Modify Expense Entry<?php } else { ?>Add Expense Entry<?php }?></li>
                </ul>
                <!-- END BREADCRUMB -->
                ...
                <div class="page-content-wrap">
                    ...
                    <div class="row">
                        <div class="col-md-12">
							...
							?>
                            <form class="form-horizontal" method="post"  name='frm2' action="addexpentry.php" enctype="multipart/form-data">
                            <div class="panel panel-default">
                                <div class="panel-heading">
									<?php
										if(isset($_REQUEST['exp_id']))
										{
											$d1=mysqli_query($con,"select * from expense_detail where exp_id='$_REQUEST[exp_id]'");
											$d=mysqli_fetch_row($d1);
											echo "<input type='hidden' name='exp_id' value='$_REQUEST[exp_id]'/>";
									?>
                                    <h3 class="panel-title"><strong>Modify</strong> Expense Entry</h3>
									<?php
										}
										else
										{
											...
											$d[1]=date("Y-m-d");
									?>
									 <h3 class="panel-title"><strong>Add New</strong> Expense Entry</h3>
									<?php
										}
									?>
                                </div>
                                <div class="panel-body">   
									...
									<div class="form-group">
                                     ...
                                     <div class="form-group">
                                        <label class="col-md-3 col-xs-12 control-label">Expense Ledger</label>
                                        <div class="col-md-6 col-xs-12">                                                                                            
                                            <select class="form-control" name="ledger_id" required>
                                                <option value="">--Select--</option>
												<?php
													$c1=mysqli_query($con,"select * from ledger_accounts where status=1  and group_id in (select group_id from group_master where group_name in ('Direct Expenses','Indirect Expenses')) order by name");
