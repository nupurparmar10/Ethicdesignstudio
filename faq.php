...
<div>
    ...
    <!-- main slide -->
    <?php
        $query = mysqli_query($con, "SELECT * FROM banner WHERE b_id = 16");
    ...
    <div style="background-image: url('<?php echo $banner; ?>'); " class="position-relative">
        ...
        <div class="px-4">
            <div class="text-white text-center py-51 position-relative">
                <h4 class="fs-30 fw-medium">FAQ's</h4>
            </div>
        </div>
    </div>
    <!-- end main slide -->
        ...
        <div class="container">
            <div class="my-4">
                ...
                <div class="accordion" id="faqAccordion">
                    <?php
                        $faqQuery = mysqli_query($con, "SELECT * FROM faq ORDER BY f_id");
                        if ($faqQuery && mysqli_num_rows($faqQuery) > 0) {
                            $faqCount = 0;
                            while ($faq = mysqli_fetch_assoc($faqQuery)) {
                                $faqCount++;
                                $headingId = "faq-heading-" . $faqCount;
                                $collapseId = "faq-collapse-" . $faqCount;
                    ?>
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="<?php echo $headingId; ?>">
                            <button class="accordion-button <?php echo ($faqCount == 1) ? '' : 'collapsed'; ?>" type="button" data-bs-toggle="collapse" data-bs-target="#<?php echo $collapseId; ?>" aria-expanded="<?php echo ($faqCount == 1) ? 'true' : 'false'; ?>" aria-controls="<?php echo $collapseId; ?>">
                                <?php echo htmlspecialchars($faq['title']); ?>
                            </button>
                        </h2>
                        <div id="<?php echo $collapseId; ?>" class="accordion-collapse collapse <?php echo ($faqCount == 1) ? 'show' : ''; ?>" aria-labelledby="<?php echo $headingId; ?>" data-bs-parent="#faqAccordion">
                    ...
                    ?>
                    <p class="text-center text-muted mb-0">No FAQ's found.</p>
                    <?php } ?>
                </div>
            </div>
        </div>
